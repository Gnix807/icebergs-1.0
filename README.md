...
已经使用旧版 `.env` 部署过的服务器可以继续沿用该文件：脚本会优先读取
`.env.production`，不存在时自动回退到 `.env`，不会覆盖或复制现有密钥。
...
请先手动拉取并确认服务器已经位于最新 `main`。部署脚本会再次执行仅快进
同步校验，然后依次完成：校验生产变量 → 等待数据库 → 在 `./backups/`
创建部署前备份 → 停止现有 Docker 服务（保留数据卷）→ 复用缓存重建镜像
→ 启动全部服务 → 执行无破坏性 Schema 同步 → 创建全文索引 → 幂等回填
版本库与贡献档案 → 校验数据 → 等待健康检查通过。检测到可能丢失数据的
Schema 变更时，部署会停止，不会自动使用 `--accept-data-loss`；停服后的
构建如果失败，脚本会尝试使用现有镜像重新拉起服务。
