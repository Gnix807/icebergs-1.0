...
export async function ALL(event: APIContext) {
  ...
  if (dataParam) {
    ...
    if (body.action === 'transfer') { if (project.creatorId !== session.userId) return json(error(ErrorCodes.FORBIDDEN, '只有创建者可转移'), 403); await prisma.project.update({ where: { id: project.id }, data: { creatorId: body.userId } }); return json(success({ transferred: true }), 200); }
