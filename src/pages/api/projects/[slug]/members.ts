...
export async function POST(event: APIContext) {
  ...
  if (existing) {
    ...
    await prisma.projectMember.create({ data: { projectId: project.id, userId: session.userId } });
  }
...
export async function PATCH(event: APIContext) {
  ...
  if (body.action === 'kick') {
    ...
    return json(success({ kicked: true }), 200);
  }
  if (body.action === 'promote') {
    await prisma.projectMember.update({ where: { id: member.id }, data: { role: 'MODERATOR' } });
    return json(success({ promoted: true }), 200);
  }
  if (body.action === 'demote') {
    if (member.userId === project.creatorId) return json(error(ErrorCodes.FORBIDDEN, '不能降级创建者'), 403);
    await prisma.projectMember.update({ where: { id: member.id }, data: { role: 'MEMBER' } });
    return json(success({ demoted: true }), 200);
  }
  if (body.action === 'transfer') {
    if (!session.isFounder && session.role !== 'ADMIN' && project.creatorId !== session.userId) return json(error(ErrorCodes.FORBIDDEN, '只有创建者可转移'), 403);
    await prisma.project.update({ where: { id: project.id }, data: { creatorId: body.userId } });
    return json(success({ transferred: true }), 200);
  }
