...
export async function GET(event: APIContext) {
  ...
  if (!taskId) return json(error(ErrorCodes.BAD_REQUEST, '缺少 taskId'), 400);
...
export async function POST(event: APIContext) {
  ...
  if (!taskId || !content || content.length > 2000) return json(error(ErrorCodes.BAD_REQUEST, '参数错误'), 400);
